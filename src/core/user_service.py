...
def generate_weekly_plan_for_user(
    ...
    Supports two modes:
        - User Mode: Load the saved user information through user_id and save the generated information in the user weekly plan.
        - Visitor Mode: Use the temporarily passed visitor_profile.
    Return the dictionary containing mode, user_profile, targets, strategy_name, weekly_plan.
    """
    ...
    try:
        ...
        # 第二步：根据用户信息计算目标
        # Step 2: Calculate the target based on the user information
        ...
        # Step 3: Obtain the strategy based on the name. If it is empty, use random strategy. At the current stage, only the strategy is parsed.
        strategy = get_strategy(strategy_name)

        # 第四步：生成weekly meal plan （测试用）
        # Step 4: Generate the weekly meal plan (for testing)
        ...
        # Step 5: Save the weekly plan only in user mode
        if mode == "user":
...
def save_weekly_plan(user_id, weekly_plan, conn):
    ...
    Save the generated weekly meal plan as a snapshot of the user's weekly plan.
    """
...
def load_latest_weekly_plan(user_id, conn):
    ...
    如果不存在则返回None。
    Read the most recent weekly meal plan generated by the specified user.
